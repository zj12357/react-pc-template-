...
function loadImages(
    input: Input | ArrayLike<Input>,
    attributes: Attributes = {},
): ImagePromise | Promise<Output[]> {
    ...
    if (isArrayLike(input)) {
        const reflect = (img: Input): Promise<HTMLImageElement | Error> =>
            loadImages(img, attributes).catch((error: Error) => error);
        const reflected = [].map.call(input, reflect) as Array<
            HTMLImageElement | Error
        >;
        const tsFix = Promise.all(reflected).then(
            (results: Array<HTMLImageElement | Error>) => {
                const loaded = results.filter(
                    (x: any): x is HTMLImageElement => x.naturalWidth,
                );
                ...
                return Promise.reject({
                    ...
                    errored: results.filter(
                        (x: any): x is Error => !x.naturalWidth,
                    ),
                });
            },
        );
    ...
    const tsFix = Promise.reject(
        new TypeError(
            'input is not an image, a URL string, or an array of them.',
        ),
    );
    return tsFix;
}
